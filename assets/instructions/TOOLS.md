...
- Default region is `__AWS_REGION__`. **Query only this one region by default.** Do NOT loop over all regions on a normal question — that is slow and can stall the bot. Only scan additional regions when the user explicitly asks (e.g. "all regions", "globally", "every region") or names a specific region.
